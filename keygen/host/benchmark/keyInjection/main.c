...
int keyInjection(okey *o,size_t key_size)
{
	TEEC_Result res;
	uint32_t flags;
	uint8_t key_filename[256]={0},key_buffer[32]={0};
	...
	sprintf((char*)key_filename,"%ld",gCount++);
        res = keyInject(o,PRIVATE,(char*)key_filename,key_buffer,key_size,flags);
        if(res!=TEEC_SUCCESS)
		return -1;
...
int main(int argc, char *argv[])
{
	...
	if(argc>1){
		...
		printf("Key Injection benchmark test\n");
		printf("\nusage: kinj <key size> <loop>\n");
	...
	for(i=0;i<loop;i++) {
		if(keyInjection(&o,keySize)!=0) {
               		printf("error found in keyInjection\n");
			closeSession(&o);
			finalizeContext(&o);
			return -1;
		}
